...
class HorsesList extends Component {
    ...
    render() {
        let elementsToDisplay = this.state.horses.map(horse => (
            <Horse clicked={()=>this.onClickHorse(horse.id_kon)} imie={horse.imie} rasa={horse.rasa} ur={horse.data_ur} w_imie={horse.w_imie} nazwisko={horse.nazwisko} key={horse.id_kon} />
        ) );
        return(
